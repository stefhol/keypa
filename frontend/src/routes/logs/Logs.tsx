...
export interface RequestLogsProps {
    logData: Log[]
}
export const RequestLogs: React.FC<RequestLogsProps> = ({ logData }) => {
...
interface KeycardUsageHistoryProps {
    keycardUsageHistory: KeycardUsageHistory[]
}
export const KeycardUsageHistoryFc: React.FC<KeycardUsageHistoryProps> = ({ keycardUsageHistory }) => {
    const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>(
        []
    )
    ...
    React.useEffect(() => {
        setColumnFilters(prev => {
            ...
            if (room_index !== -1) {
                column[room_index].value = filter.room_id
            } else {
